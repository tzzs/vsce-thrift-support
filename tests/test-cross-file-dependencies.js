...
// Enhanced vscode mock with file system operations
const vscode = {
  DiagnosticSeverity: {
    Error: 0,
    Warning: 1,
...
const mockPath = {
  ...
  dirname: (filePath) => {
    if (filePath.includes('test-thrift')) {
      return 'e:/workspaces/trae/trae2/thrift-support2/test-thrift';
    }
    return path.dirname(filePath);
  },
  resolve: (dir, file) => {
    if (dir.includes('test-thrift')) {
      return `e:/workspaces/trae/trae2/thrift-support2/test-thrift/${file}`;
    }
...
const mockFs = {
  existsSync: (filePath) => {
    return vscode.workspace.textDocuments.some(doc => 
      doc.uri.fsPath === filePath || doc.uri.toString() === filePath
    );
  }
};
