...
const {Position, Range, Location, Uri, workspace: baseWorkspace} = require('./mock-vscode');
...
const workspace = {
    ...
    async findFiles(glob) {
        ...
        return files.map(f => Uri.file(path.join(dir, f)));
    },
    fs: {
        ...baseWorkspace.fs,
        async stat(uri) {
...
async function testDuplicateIncludeNavigatesToFirst() {
    ...
    // Check if range exists and has the expected structure
    if (loc.range && loc.range.start) {
