...
import {CacheManager} from '@tanzz/thrift-core'; // Import the new cache manager
import {createLineRange, rangeIntersectsLineRange} from '../utils/line-range';
import {BlockCache, BlockCacheValue, MemberCache, MemberCacheByBlock, MemberCacheValue, ThriftIssue} from '@tanzz/thrift-core';
import {hashText} from '@tanzz/thrift-core';
...
export function buildMemberCache(ast: nodes.ThriftDocument, lines: string[], issues: ThriftIssue[]) {
    ...
    for (const node of ast.body) {
        const blockKey = makeLineRangeKey(createLineRange(node.range.start.line, node.range.end.line));
...
export function buildMemberCacheForNode(
    ...
    let members: Array<{range: {start: {line: number; character: number}; end: {line: number; character: number}}}> = [];
    ...
    for (const member of members) {
        ...
        const memberIssues = issues.filter(issue => rangeIntersectsLineRange(issue.range, createLineRange(startLine, endLine)));
        const key = makeLineRangeKey(createLineRange(startLine, endLine));
        cache.set(key, {
            range: createLineRange(startLine, endLine),
...
export function buildBlockCache(ast: nodes.ThriftDocument, lines: string[], issues: ThriftIssue[]) {
    ...
    for (const node of ast.body) {
        ...
        const blockIssues = issues.filter(issue => rangeIntersectsLineRange(issue.range, createLineRange(startLine, endLine)));
