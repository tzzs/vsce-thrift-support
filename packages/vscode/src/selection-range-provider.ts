...
import {findSmallestNodeAtPosition, nodePathFromLeaf, positionInRange} from '@tanzz/thrift-core';
...
export class ThriftSelectionRangeProvider implements vscode.SelectionRangeProvider {
    ...
    private getSelectionRanges(document: vscode.TextDocument, position: vscode.Position): vscode.SelectionRange | undefined {
        ...
        for (const node of path) {
            ...
            const clampedRange = this.clampRange(document, toVscodeRange(node.range));
        ...
        if (node.type === nodes.ThriftNodeType.Field) {
            ...
            if (nameRange) {
                const betweenTypeAndName = typeRange
                    ...
                    : false;
                if (this.isBetween(position, nameRange.start, nameRange.end) || betweenTypeAndName) {
                    return nameRange;
                }
            }

            if (requiredRange && this.isBetween(position, requiredRange.start, requiredRange.end)) {
        ...
        if (node.type === nodes.ThriftNodeType.Function) {
            ...
            if (nameRange) {
                const betweenReturnAndName = returnRange
                    ...
                    : false;
                if (this.isBetween(position, nameRange.start, nameRange.end) || betweenReturnAndName) {
                    return nameRange;
                }
            }

            if (returnRange && this.isBetween(position, returnRange.start, returnRange.end)) {
                return returnRange;
            }
        }
        ...
        if (node.type === nodes.ThriftNodeType.Include) {
            ...
            if (match && match.index !== undefined) {
                ...
                if (valueIndex >= 0) {
                    ...
                    if (this.isBetween(position, valueRange.start, valueRange.end)) {
                        return valueRange;
                    }
                }
            }
