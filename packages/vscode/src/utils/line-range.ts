...
type AnyRange = {start: {line: number}; end: {line: number}};

export function rangeIntersectsLineRange(range: AnyRange, lineRange: LineRange): boolean {
    return range.start.line <= lineRange.endLine && range.end.line >= lineRange.startLine;
}

export function rangeContainsLineRange(range: AnyRange, lineRange: LineRange): boolean {
