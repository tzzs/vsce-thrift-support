...
export function parseFieldList(text: string, baseLine: number, baseChar: number): nodes.Field[] {
    ...
    for (const seg of segments) {
        ...
        const field = createField({
            range: new Range(startPos.line, startPos.char, endPos.line, endPos.char),
            nameRange: new Range(nameStart.line, nameStart.char, nameEnd.line, nameEnd.char),
            typeRange: new Range(typeStart.line, typeStart.char, typeEnd.line, typeEnd.char),
            id: parseInt(tokens[idIndex].value, 10),
            requiredness: requiredness ?? 'required',
            fieldType,
            name: nameToken.value,
            defaultValue: defaultInfo?.value,
            defaultValueRange: defaultStart && defaultEnd ? new Range(defaultStart.line, defaultStart.char, defaultEnd.line, defaultEnd.char) : undefined
        });
        fields.push(field);
    }
    return fields;
