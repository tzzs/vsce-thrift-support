...
export function buildConstValueRange(
    ...
    const range = new Range(start.line, start.char, end.line, end.char);
...
export function findInitializerRange(
    ...
    return new Range(lineNumber, startChar, lineNumber, endChar);
}
...
export function findWordRangeInLine(
    ...
    while ((match = regex.exec(line)) !== null) {
        if (match.index >= searchStart) {
            return new Range(lineNumber, match.index, lineNumber, match.index + word.length);
        }
    }
    return undefined;
}
...
export function findTypeRangeInLine(
    ...
    if (idx >= 0) {
        return new Range(lineNumber, idx, lineNumber, idx + typeText.length);
    }
    return undefined;
}
