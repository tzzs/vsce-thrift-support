...
export function parseFieldList(text: string, baseLine: number, baseChar: number): nodes.Field[] {
    ...
    for (const seg of segments) {
        ...
        const nameOffset = findWordOffset(segmentText, match[4]);
        ...
        const nameStart = nameOffset !== null ? offsetToPosition(text, baseLine, baseChar, segmentStart + nameOffset) : null;
        const nameEnd = nameOffset !== null ? offsetToPosition(text, baseLine, baseChar, segmentStart + nameOffset + match[4].length) : null;
        const typeStart = typeOffset >= 0 ? offsetToPosition(text, baseLine, baseChar, segmentStart + typeOffset) : null;
        const typeEnd = typeOffset >= 0 ? offsetToPosition(text, baseLine, baseChar, segmentStart + typeOffset + match[3].trim().length) : null;
