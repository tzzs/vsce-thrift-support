...
for (let i = 0; i < line.length; i++) {
    ...
    if (ch === '{' || ch === '(' || ch === '<') {
        events.push({i, ch, kind: 'open'});
    }
    if (ch === '}' || ch === ')' || ch === '>') {
        events.push({i, ch, kind: 'close'});
    }
}
...
for (let ln = 0; ln < lines.length; ln++) {
    ...
    for (let i = 0; i < lineText.length; i++) {
        ...
        if (c === '{' || c === '(' || c === '<') {
            globalEvents.push({ln, i, ch: c, kind: 'open'});
        }
        if (c === '}' || c === ')' || c === '>') {
            globalEvents.push({ln, i, ch: c, kind: 'close'});
        }
    }
}
...
for (let i = 0; i < traceLine.length; i++) {
    ...
    if (sInS) {
        if (!sEsc && c === '\\') {
            sEsc = true;
            trace.push({i, c, evt: 'esc'});
            continue;
        }
        if (!sEsc && c === '\'') {
            sInS = false;
            trace.push({i, c, evt: 'exitS'});
        }
        sEsc = false;
        continue;
    }
    ...
    if (c === '\'') {
        sInS = true;
        trace.push({i, c, evt: 'enterS'});
        continue;
    }
    if (c === '"') {
        sInD = true;
        trace.push({i, c, evt: 'enterD'});
        continue;
    }
}
