...
mockVscode.Uri = {
    ...
    parse: (value) => new mockVscode.Uri('file', '', value, '', '')
};
...
async function debugTypeReferences() {
    console.log('Debugging type references...');
...
service UserService {
  ...
  void createUser(1: User user)
    ...
    // Check what word is found at the position
    const wordRange = document.getWordRangeAtPosition(position);
    if (wordRange) {
        const symbolName = document.getText(wordRange);
        console.log(`Found word at position: "${symbolName}"`);
    ...
    references.forEach((ref, index) => {
        console.log(`  ${index + 1}. ${ref.uri.fsPath} [${ref.range.start.line}, ${ref.range.start.character}] to [${ref.range.end.line}, ${ref.range.end.character}]`);
    });
}

debugTypeReferences().catch(console.error);