...
async function debugFindReferencesInDocument() {
    console.log('\n--- 测试 findReferencesInDocument 方法 ---');
    ...
    try {
        ...
        // 详细显示每个引用的位置
        if (mainReferences.length > 0) {
            mainReferences.forEach((ref, index) => {
...
async function debugProvideReferences() {
...
async function main() {
    try {
        await debugFindReferencesInDocument();
        await debugProvideReferences();
    } catch (error) {
